# LeetCode Practice · Java

My collection of **Java solutions to LeetCode problems**, organized into individual problem folders. I use this repository to practice problem solving and revisit data structures and algorithms.

## Browse by topic

These starting points group problems by the concepts they explore. Individual solutions may use different approaches.

| Topic | Selected problems |
| --- | --- |
| Arrays | [Remove Element](0027-remove-element/) · [Move Zeroes](0283-move-zeroes/) · [Merge Sorted Array](0088-merge-sorted-array/) |
| Strings | [Reverse String](0344-reverse-string/) · [Valid Anagram](0242-valid-anagram/) · [Merge Strings Alternately](1768-merge-strings-alternately/) |
| Searching | [Binary Search](0704-binary-search/) · [Search Insert Position](0035-search-insert-position/) |
| Two pointers | [Container With Most Water](0011-container-with-most-water/) · [3Sum](0015-3sum/) · [Valid Palindrome](0125-valid-palindrome/) |
| Sliding windows | [Maximum Average Subarray I](0643-maximum-average-subarray-i/) · [Minimum Size Subarray Sum](0209-minimum-size-subarray-sum/) · [Max Consecutive Ones III](1004-max-consecutive-ones-iii/) |
| Prefix sums and subarrays | [Running Sum of 1D Array](1480-running-sum-of-1d-array/) · [Subarray Sum Equals K](0560-subarray-sum-equals-k/) · [Maximum Subarray](0053-maximum-subarray/) |
| Hashing and counting | [Two Sum](0001-two-sum/) · [Contains Duplicate](0217-contains-duplicate/) · [Majority Element](0169-majority-element/) |
| Stacks | [Valid Parentheses](0020-valid-parentheses/) |
| Dynamic programming | [Climbing Stairs](0070-climbing-stairs/) · [Min Cost Climbing Stairs](0746-min-cost-climbing-stairs/) |
| Math and bit manipulation | [Reverse Integer](0007-reverse-integer/) · [Count Primes](0204-count-primes/) · [Single Number](0136-single-number/) |

## How to use this repository

1. Open a problem folder and read its `README.md`.
2. Review the corresponding `.java` solution.
3. To run a judge-style solution, choose **Java** on the matching LeetCode problem and use its provided method signature.
4. For local execution, supply an appropriate test harness and inputs. Judge-style `Solution` classes are not standalone applications.

## Organization

Each problem directory is named `problem-number-problem-name` and contains the Java solution plus its problem statement.

<details>
<summary><strong>View the problem index</strong></summary>

| Problem # | Problem |
| --- | --- |
| 1 | [Two Sum](0001-two-sum/) |
| 4 | [Median Of Two Sorted Arrays](0004-median-of-two-sorted-arrays/) |
| 6 | [Zigzag Conversion](0006-zigzag-conversion/) |
| 7 | [Reverse Integer](0007-reverse-integer/) |
| 9 | [Palindrome Number](0009-palindrome-number/) |
| 11 | [Container With Most Water](0011-container-with-most-water/) |
| 13 | [Roman To Integer](0013-roman-to-integer/) |
| 15 | [3Sum](0015-3sum/) |
| 20 | [Valid Parentheses](0020-valid-parentheses/) |
| 27 | [Remove Element](0027-remove-element/) |
| 28 | [Find The Index Of The First Occurrence In A String](0028-find-the-index-of-the-first-occurrence-in-a-string/) |
| 35 | [Search Insert Position](0035-search-insert-position/) |
| 53 | [Maximum Subarray](0053-maximum-subarray/) |
| 58 | [Length Of Last Word](0058-length-of-last-word/) |
| 66 | [Plus One](0066-plus-one/) |
| 69 | [Sqrtx](0069-sqrtx/) |
| 70 | [Climbing Stairs](0070-climbing-stairs/) |
| 88 | [Merge Sorted Array](0088-merge-sorted-array/) |
| 121 | [Best Time To Buy And Sell Stock](0121-best-time-to-buy-and-sell-stock/) |
| 125 | [Valid Palindrome](0125-valid-palindrome/) |
| 136 | [Single Number](0136-single-number/) |
| 137 | [Single Number II](0137-single-number-ii/) |
| 169 | [Majority Element](0169-majority-element/) |
| 204 | [Count Primes](0204-count-primes/) |
| 209 | [Minimum Size Subarray Sum](0209-minimum-size-subarray-sum/) |
| 215 | [Kth Largest Element In An Array](0215-kth-largest-element-in-an-array/) |
| 217 | [Contains Duplicate](0217-contains-duplicate/) |
| 242 | [Valid Anagram](0242-valid-anagram/) |
| 258 | [Add Digits](0258-add-digits/) |
| 268 | [Missing Number](0268-missing-number/) |
| 283 | [Move Zeroes](0283-move-zeroes/) |
| 342 | [Power Of Four](0342-power-of-four/) |
| 344 | [Reverse String](0344-reverse-string/) |
| 389 | [Find The Difference](0389-find-the-difference/) |
| 392 | [Is Subsequence](0392-is-subsequence/) |
| 560 | [Subarray Sum Equals K](0560-subarray-sum-equals-k/) |
| 567 | [Permutation In String](0567-permutation-in-string/) |
| 643 | [Maximum Average Subarray I](0643-maximum-average-subarray-i/) |
| 704 | [Binary Search](0704-binary-search/) |
| 713 | [Subarray Product Less Than K](0713-subarray-product-less-than-k/) |
| 746 | [Min Cost Climbing Stairs](0746-min-cost-climbing-stairs/) |
| 1004 | [Max Consecutive Ones III](1004-max-consecutive-ones-iii/) |
| 1456 | [Maximum Number Of Vowels In A Substring Of Given Length](1456-maximum-number-of-vowels-in-a-substring-of-given-length/) |
| 1470 | [Shuffle The Array](1470-shuffle-the-array/) |
| 1480 | [Running Sum Of 1D Array](1480-running-sum-of-1d-array/) |
| 1493 | [Longest Subarray Of 1s After Deleting One Element](1493-longest-subarray-of-1s-after-deleting-one-element/) |
| 1662 | [Check If Two String Arrays Are Equivalent](1662-check-if-two-string-arrays-are-equivalent/) |
| 1768 | [Merge Strings Alternately](1768-merge-strings-alternately/) |
| 1920 | [Build Array From Permutation](1920-build-array-from-permutation/) |
| 1929 | [Concatenation Of Array](1929-concatenation-of-array/) |
| 1979 | [Find Greatest Common Divisor Of Array](1979-find-greatest-common-divisor-of-array/) |
| 3005 | [Count Elements With Maximum Frequency](3005-count-elements-with-maximum-frequency/) |
| 3090 | [Maximum Length Substring With Two Occurrences](3090-maximum-length-substring-with-two-occurrences/) |
| 3345 | [Smallest Divisible Digit Product I](3345-smallest-divisible-digit-product-i/) |
| 3622 | [Check Divisibility By Digit Sum And Product](3622-check-divisibility-by-digit-sum-and-product/) |

</details>

## Practice approach

I use these solutions as a record of learning. When revisiting a problem, I focus on the reasoning, edge cases, time complexity and space complexity, then compare alternative approaches.

Problem statements belong to their respective owners. This repository is for personal study and practice.

[Neeraj Baghel](https://github.com/radhe-8075)

<!---LeetCode Topics Start-->
# LeetCode Topics
## Array
| Problem Name | Difficulty |
| ------- | ------- |
| [0053-maximum-subarray](https://github.com/radhe-8075/Leetcode/tree/main/0053-maximum-subarray/) | Medium |
| [0485-max-consecutive-ones](https://github.com/radhe-8075/Leetcode/tree/main/0485-max-consecutive-ones/) | Easy |
| [1470-shuffle-the-array](https://github.com/radhe-8075/Leetcode/tree/main/1470-shuffle-the-array/) | Easy |
| [1929-concatenation-of-array](https://github.com/radhe-8075/Leetcode/tree/main/1929-concatenation-of-array/) | Easy |
## Divide and Conquer
| Problem Name | Difficulty |
| ------- | ------- |
| [0053-maximum-subarray](https://github.com/radhe-8075/Leetcode/tree/main/0053-maximum-subarray/) | Medium |
## Dynamic Programming
| Problem Name | Difficulty |
| ------- | ------- |
| [0053-maximum-subarray](https://github.com/radhe-8075/Leetcode/tree/main/0053-maximum-subarray/) | Medium |
## Simulation
| Problem Name | Difficulty |
| ------- | ------- |
| [1929-concatenation-of-array](https://github.com/radhe-8075/Leetcode/tree/main/1929-concatenation-of-array/) | Easy |
<!---LeetCode Topics End-->